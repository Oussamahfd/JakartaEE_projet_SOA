	...
	<section class="home-section">
		...
		<main
			...
			<div
				...
				<span>Home/Emploi</span><span style="color: #2596be;">/</span><span
					style="color: #2596be; text-decoration: underline;">Profs</span>
			</div>
			<div class="search-container" style="margin-left: 59%;">
			<form style="display: flex; align-items: center;" action="Serchprofemplois"  method="post">
				<select class="education-level" name="idprof">
				<%
				List<Prof> etudList = new ArrayList<>();
				while(rs.next()){ 
				String np=rs.getString("nom") ;
				String prp=rs.getString("prenom");
				int idprfsd=rs.getInt("user_id");
				Prof prfs = new Prof(np, prp,idprfsd);
		        etudList.add(prfs);}
				%>
				<%session.setAttribute("lprf", etudList); 
				for(Prof val : etudList){
				%>
					<option class="opt" value="<%=val.getId()%>"><%=val.getPrenom() %> <%= esspace%> <%=val.getNom() %></option>
					<%} %>
				</select> 
														...
														<tbody class="fc-body">
															...
															<tr>
																...
																<td style="position: relative;">
																	<div class="date"><%=day%></div> <a
																	class="fc-day-grid-event fc-h-event fc-event fc-start fc-end fc-draggable fc-resizable"
																	style="background-color: <%=colorList.get(1)%>; border-color: <%=colorList.get(1)%>;">
