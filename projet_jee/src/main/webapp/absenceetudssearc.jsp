	...
	<section class="home-section">
		...
		<main
			...
			<div class="search-container" style="margin-left: 59%;">
				<form style="display: flex; align-items: center;" action="searchetudsabs"  method="post">

        <select class="education-level" name="niv" style="margin-right: 10px;">
            
            <option class="opt" value="2-bac-pc">2-bac-pc</option>
            <option class="opt" value="2-bac-sm">2-bac-sm</option>        
        ...
        <select class="education-level" name="grp" style="margin-right: 10px;">
					...
					<div class="col-12">
						<div class="card my-4" style="border-radius: 12px;">
							<div
								...
								<div class=" shadow p-3 border-radius-lg pt-4 pb-3"
									style="BACKGROUND-COLOR: BLACK;">
									<h6 class="text-white text-capitalize ps-3"
										style="text-align: center; font-size: 18pt;">
										<b>Table D'absence(Etudiants)</b>
									</h6>
								</div>
							</div>
									...
									<table class="table align-items-center mb-0">
										<thead>
											<tr>
												...
												<th
													class="text-uppercase text-secondary text-xxs font-weight-bolder opacity-7">Nom
													complet</th>


												<th
													class="text-center text-uppercase text-secondary text-xxs font-weight-bolder opacity-7">octorbe</th>
												<th
													class="text-center text-uppercase text-secondary text-xxs font-weight-bolder opacity-7">novembre</th>
												<th
													class="text-center text-uppercase text-secondary text-xxs font-weight-bolder opacity-7">decembre</th>
												<th
													class="text-center text-uppercase text-secondary text-xxs font-weight-bolder opacity-7">janvier</th>
												<th
													class="text-center text-uppercase text-secondary text-xxs font-weight-bolder opacity-7">fevrier</th>
												<th
													class="text-center text-uppercase text-secondary text-xxs font-weight-bolder opacity-7">mars</th>
												<th
													class="text-center text-uppercase text-secondary text-xxs font-weight-bolder opacity-7">avril</th>
												<th
													class="text-center text-uppercase text-secondary text-xxs font-weight-bolder opacity-7">mai</th>

												<th
													class="text-center text-uppercase text-secondary text-xxs font-weight-bolder opacity-7">juin</th>
												<th class="text-secondary opacity-7"></th>
												<th class="text-secondary opacity-7"></th>
											</tr>
										</thead>
										<%
										String nv = (String) session.getAttribute("nv");
										ResultSet rs = et.serchalletrabs(nv);
										String es = " ";
										int cmp = 0;
										List<Absence> etudList = new ArrayList<>();
										while (rs.next()) {
											String nom = rs.getString("nom");
											String prenom = rs.getString("prenom");

											int idpai = rs.getInt("user_id");
											int oct = rs.getInt("octorbe");
											int nov = rs.getInt("novembre");
											int dec = rs.getInt("decembre");
											int jan = rs.getInt("janvier");
											int fev = rs.getInt("fevrier");
											int mar = rs.getInt("mars");
											int avr = rs.getInt("avril");
											int mai = rs.getInt("mai");
											int jui = rs.getInt("juin");

											Absence etud = new Absence(nom, prenom, idpai, oct, nov, dec, jan, fev, mar, avr, mai, jui);
											etudList.add(etud);
											cmp++;
										}
												...
												<td>
													<div class="d-flex px-2 py-1">
														...
														<div class="d-flex flex-column justify-content-center">
															...
															<p class="text-center text-xs text-secondary mb-0"></p>
														</div>
													</div>
												</td>
												<td>
													<p class="text-center text-xs font-weight-bold mb-0"><%=paim.getOct()%></p>

												</td>
												<td>
													<p class="text-center text-xs font-weight-bold mb-0"><%=paim.getNov()%></p>

												</td>

												<td>
													<p class="text-center text-xs font-weight-bold mb-0"><%=paim.getDec()%></p>

												</td>
												<td>
													<p class="text-center text-xs font-weight-bold mb-0"><%=paim.getJan()%></p>

												</td>
												<td>
													<p class="text-center text-xs font-weight-bold mb-0"><%=paim.getFev()%></p>

												</td>
												<td>
													<p class="text-center text-xs font-weight-bold mb-0"><%=paim.getMar()%></p>

												</td>
												<td>
													<p class="text-center text-xs font-weight-bold mb-0"><%=paim.getAvr()%></p>

												</td>
												<td>
													<p class="text-center text-xs font-weight-bold mb-0"><%=paim.getMai()%></p>

												</td>
												<td>
													<p class="text-center text-xs font-weight-bold mb-0"><%=paim.getJun()%></p>

												</td>


												<td class="align-middle" style="color: green;">
													<form action="Ajouterabsetuds.jsp" method="post">
														<input type="hidden" name="userIddd"
															value="<%=paim.getId()%>">
														<button type="submit"
															class="text-secondary font-weight-bold green text-xs"
															style="color: green;" data-toggle="tooltip"
															data-original-title="Edit user">Edit</button>
													</form>
												</td>
					...
					<div class="col-lg-6 mb-lg-0 mb-4">
						<div
							...
							made with <i class="fa fa-heart" aria-hidden="true"></i> by <a
								...
								target="_blank">Creative Tim<%=etudList.size() %></a> for a better web.
						</div>
					</div>
					<div class="col-lg-6">
						<ul
