...
<body class="g-sidenav-show  bg-gray-200">
	...
	<section class="home-section">
		<div class="home-content">
			...
			<div>
				<a href="Ajouterprofsgest.jsp"><button>
						<i class="bi bi-plus-square-fill"></i> Ajouter Profs
					</button></a>
			</div>
		</div>
		...
		<main
			...
			<div style="font-family: 'Red Hat Display', sans-serif;display: flex;align-items: center;margin-left: 73%;font-size:14pt;margin-bottom:13px;"><span>Home/Gestion</span><span style="color: #2596be;">/</span><span style="color: #2596be; text-decoration: underline;">Profs</span></div>
			 <div class="search-container" style="margin-left: 59%;">
         <form style="display: flex; align-items: center;" action="Serchprofgets"  method="post">
						...
						<div class="card my-4" style="border-radius:12px;">
							<div
								...
								<div class=" shadow p-3 border-radius-lg pt-4 pb-3" style="BACKGROUND-COLOR:BLACK;">
									<h6 class="text-white text-capitalize ps-3" style="text-align:center;font-size:18pt;"><b>Table Des profs</b></h6>
								</div>
							</div>
	...
	<% ResultSet rs = et.allprofs();  String es=" ";
	int cmp =0;
	List<Prof> etudList = new ArrayList<>();
	while (rs.next()) {
        ...
        String nv = rs.getString("niveau");
        Float sal=rs.getFloat("salaire");
        int idprfs = rs.getInt("user_id");
        

        Prof prfs = new Prof(nom, prenom, email, tel, role, sal,nv,idprfs);
        etudList.add(prfs);
        cmp++;
    }
...
%>
											...
											<tr>
											<td>
													<p class="text-center text-xs font-weight-bold mb-0"><%= prfsty.getId() %></p>
												...
												<td>
													<div class="d-flex px-2 py-1">
														...
														<div class="d-flex flex-column justify-content-center">
															<h6 class="text-center mb-0 text-sm"><%=prfsty.getNom() %><%=es %><%=prfsty.getPrenom() %></h6>
															<p class="text-center text-xs text-secondary mb-0"><%=prfsty.getEmail() %></p>
														</div>
													</div>
												</td>
												<td>
													<p class="text-center text-xs font-weight-bold mb-0"><%=prfsty.getRole() %></p>
													
												</td>
												<td>
													<p class="text-center text-xs font-weight-bold mb-0"><%= prfsty.getNv() %></p>
													
												</td>
												
												<td>
													<p class="text-center text-xs font-weight-bold mb-0"><%=prfsty.getSal() %></p>
													
												</td>
																			
											
												<td class="align-middle" style="color: green;">
    <form action="Editprf.jsp" method="post">
        <input type="hidden" name="userIddd" value="<%= prfsty.getId() %>">
